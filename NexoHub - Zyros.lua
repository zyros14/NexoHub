--[[
    Blox Fruits Hub
    Features: Auto Farm Level, Auto Farm Mastery, Auto Farm Nearest,
              Auto Farm Bosses, Auto Chest, Auto Sea Beasts, Teleports
]]
...
local Window = Library.CreateLib("Blox Fruits Hub", "Sentinel")
...
getgenv().Settings = {
    ...
    AttackDistance = 60,
    TweenSpeed = 300,
    FarmMode = "Questline", -- Questline, Nearest, Bosses
...
function Utils.GetPlayerLevel()
    local level = Player:GetAttribute("Level") or Player:FindFirstChild("Data") and Player.Data:FindFirstChild("Level") and Player.Data.Level.Value
    if not level then
...
function Utils.GetNearestMob()
    local nearest = nil
    local dist = math.huge
    for _, mob in pairs(Workspace:WaitForChild("_WorldOrigin", 1) and Workspace._WorldOrigin:FindFirstChild("EnemySpawns") and Workspace._WorldOrigin.EnemySpawns:GetChildren() or {}) do
        if mob:IsA("BasePart") then
            local mag = Utils.DistanceFromCharacter(mob.Position)
            if mag < dist then
                dist = mag
                nearest = mob
            end
...
function Utils.HasItemInCharacter(name)
...
function Movement.GetCFrameFromInstance(instance)
    if instance:IsA("Model") then
        local primary = instance:FindFirstChild("HumanoidRootPart")
...
function Movement.TeleportToInstance(instance)
    local cf = Movement.GetCFrameFromInstance(instance)
    if cf then
        Movement.Teleport(cf)
        return true
    end
    return false
end

function Movement.TweenToInstance(instance)
    local cf = Movement.GetCFrameFromInstance(instance)
    if cf then
        return Movement.TweenTo(cf)
    end
    return nil
end
...
function Combat.KillAura(distance)
    distance = distance or 500
    for _, enemy in pairs(Utils.GetEnemiesInWorkspace()) do
        if Utils.IsEnemyAlive(enemy) then
            local pp = enemy.HumanoidRootPart
            if Utils.DistanceFromCharacter(pp.Position) < distance then
...
-- Second Sea Quests (Level 700-1500)
QuestData.Sea2 = {
...
-- Third Sea Quests (Level 1500-2550)
QuestData.Sea3 = {
    ...
    {Level = 2375, Isle = "Isle Champion", QuestName = "TikiQuest2", QuestNumber = 2, CFrameQuest = CFrame.new(-16542, 56, -179), CFrameMob = CFrame.new(-16542, 56, -179)},
...
function AutoFarmLevel.StartQuest(quest)
    ...
    if questGui and questGui.Visible then
        ...
        if title and title.Text:find(quest.Name) then
            return true -- Already on this quest
        end
    end

    -- Teleport to quest NPC and start quest
...
function AutoFarmLevel.FarmMob(quest)
    ...
    -- Teleport above mob
    local mobCF = mob.HumanoidRootPart.CFrame * CFrame.new(0, 15, 0)
    Movement.Teleport(mobCF)
...
function AutoFarmMastery.GetBestMob()
    ...
    -- Find the highest level mob we can farm
    local bestQuest = nil
    for _, quest in ipairs(quests) do
        if level >= quest.Level then
            bestQuest = quest
...
FarmSection:NewToggle("Auto Farm Mastery", "Farms mastery on best mob", function(state)
    ...
    if state then
        task.spawn(AutoFarmMastery.Farm)
    end
end)
...
FarmSection:NewToggle("Auto Farm Nearest", "Farms nearest enemy", function(state)
    ...
    if state then
        task.spawn(AutoFarmNearest.Farm)
    end
end)
...
FarmSection:NewToggle("Auto Farm Bosses", "Farms all bosses in current sea", function(state)
    ...
    if state then
        task.spawn(AutoFarmBosses.Farm)
    end
end)
...
SeaSection:NewButton("Sea 1", "Go to First Sea", function()
    Teleport.ToSea("Sea 1")
end)
SeaSection:NewButton("Sea 2", "Go to Second Sea", function()
    Teleport.ToSea("Sea 2")
end)
SeaSection:NewButton("Sea 3", "Go to Third Sea", function()
    Teleport.ToSea("Sea 3")
end)

-- Player Teleport
local PlayerTPSection = TeleportTab:NewSection("Players")
local playerDropdown = PlayerTPSection:NewDropdown("Select Player", "Choose a player to teleport", function(option)
...
CreditsSection:NewLabel("Blox Fruits Hub v1.0")
CreditsSection:NewLabel("Made for educational purposes")
